...
class RNN(ModelRNNAbstract):
   ...
   #        sample_indices = [i for i in range( len(y))]
    ...
    def representative_dataset(self):
        ...
        #print(x_train)
        #print('qqqq')
        #print(tf.dtypes.cast(x_test, tf.float32))
        #print('reyyererer')
        #print(y_test[0])
        #yield [tf.dtypes.cast(x_train, tf.float32)]
        for data in tf.data.Dataset.from_tensor_slices(x_test).batch(1).take(3000):
            print(data)
            yield [tf.dtypes.cast(data, tf.float16)]
    ...
    def convert_tflite(self):
        ...
        #tflite_type: DEFAULT=0.0, SIZE_MODEL=1.0, FLOAT16_MODEL=2.0, MODEL INT ONLY=4.0
        if(tflite_type < 1.0):
            print("MODEL DEFAULT 0.0")
            self.converter.optimizations = [tf.lite.Optimize.DEFAULT]
            self.converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS, tf.lite.OpsSet.SELECT_TF_OPS]
        elif((tflite_type > 0.0) and (tflite_type < 2.0)):
            print("MODEL SIZE_MODEL 1.0")
            self.converter.optimizations = [tf.lite.Optimize.OPTIMIZE_FOR_SIZE]
            self.converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS, tf.lite.OpsSet.SELECT_TF_OPS]
        elif((tflite_type > 1.0) and (tflite_type < 3.0)):
            print("MODEL FLOAT16_MODEL 2.0")
            self.converter.optimizations = [tf.lite.Optimize.DEFAULT]
            self.converter.representative_dataset = self.representative_dataset
            self.converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS, tf.lite.OpsSet.SELECT_TF_OPS]
            self.converter.target_spec.supported_types = [tf.float16]
        elif((tflite_type > 3.0) and (tflite_type < 5.0)):
            print("MODEL INT ONLY")
            self.converter.optimizations = [tf.lite.Optimize.DEFAULT]
            self.converter.representative_dataset = self.representative_dataset
            self.converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS_INT8, tf.lite.OpsSet.SELECT_TF_OPS]
            self.converter._experimental_lower_tensor_list_ops=False
            self.converter.inference_input_type = tf.float32  # or tf.uint8
            self.converter.inference_output_type = tf.float32 # or tf.uint8
        else:
            self.converter.optimizations = [tf.lite.Optimize.DEFAULT]
            self.converter.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS, tf.lite.OpsSet.SELECT_TF_OPS]
        ...
        input_type = self.interpreter.get_input_details()[0]['dtype']
        print('input: ', input_type)
        output_type = self.interpreter.get_output_details()[0]['dtype']
