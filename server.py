    ...
    for case in case_range:
        ...
        for tau_setup in tau_setup_all:
            ...
            while True:
                ...
                # the variable for the total calculation of data _t
                data_size_total_t = 0
                ...
                # array with rejected nodes in global aggregation
                rejected_nodes = np.zeros(n_nodes)
                rejected_data = 0
                
                # τhe array for data size local _t
                data_size_local_all_t = []

                tau_actual = 0
                #calculate the Total Data first of all the clients => data_size_total_t

                #data_size_local_all_t.append(data_size_local_t)
                #data_size_total_t += data_size_local_t 
                msg_all = []
                data_size_local_arr = np.zeros(n_nodes)
                loss_local_last_global_arr = np.zeros(n_nodes)
                for n in range(0, n_nodes):
                    data_size_total_t += len(indices_each_node_case[case][n])
                    msg = recv_msg(client_sock_all[n], 'MSG_WEIGHT_TIME_SIZE_CLIENT_TO_SERVER')
                    msg_all.append(msg)
                    data_size_local_arr[n] = msg[4]
                    loss_local_last_global_arr[n] = msg[5].numpy()
                    #loss_local_last_global_arr[n] = msg[5]
                    
                # Set the thresholds according to existence of outliers
                #if loss_local_last_global_arr.mean() < np.std(loss_local_last_global_arr):
                #    thres_size1 = loss_local_last_global_arr.mean()-0.5*np.std(loss_local_last_global_arr)
                #else:
                #    thres_size1 = loss_local_last_global_arr.mean()-1.0*np.std(loss_local_last_global_arr)
                
                
                # Set the thresholds according to existence of outliers in the data size 
                if data_size_local_arr.mean() < np.std(data_size_local_arr):
                    thres_size = data_size_local_arr.mean()-0.5*np.std(data_size_local_arr)
                else:
                    thres_size = data_size_local_arr.mean()-1.0*np.std(data_size_local_arr)
            
                # Set the thresholds according to existence of outliers in the local loss
                if loss_local_last_global_arr.mean() < np.std(loss_local_last_global_arr):
                    thres_loss = loss_local_last_global_arr.mean()-0.5*np.std(loss_local_last_global_arr)
                else:
                    thres_loss = loss_local_last_global_arr.mean()-1.0*np.std(loss_local_last_global_arr)
                
                print(loss_local_last_global_arr)
                print(thres_size, thres_loss)
                
                
                
                for n,msg in enumerate(msg_all):
                    #msg = recv_msg(client_sock_all[n], 'MSG_WEIGHT_TIME_SIZE_CLIENT_TO_SERVER')
                    ...
                    # conditions for global aggregation of w parameters (for adequate data size and small local loss function for each client)    
                    if ((loss_local_last_global <= thres_loss)   and    (data_size_local >= thres_size)):
                    #if (data_size_local > thres_size):
                          ...
                          rejected_nodes [n] = 1
                          rejected_data += data_size_local
                          data_size_local_all.append(0)
                ...
                #exclude the data of node that was removed off the global aggregation calculations  
            ...
            postprocessing(w_eval)
            print(f'rejected_nodes = {rejected_nodes}')
